        ...
        <div class="row align-items-center">
            ...
            <div class="col-auto text-left box @if (!$years) mr-3 @endif">
                ...
                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="moduleDropDown">
                    <a class="dropdown-item {{ Request::is(['pasca_auditor', 'pasca_auditor/semua*']) ? 'active' : '' }}"
                        href="{{ route('pasca_home_auditor') }}">Semua data</a>
                    <a class="dropdown-item {{ Request::is('pasca_auditor/evaluasi*') ? 'active' : '' }}"
                        href="{{ route('pasca_home_auditor', 'evaluasi') }}">Evaluasi diri</a>
                    <a class="dropdown-item {{ Request::is('pasca_auditor/standar*') ? 'active' : '' }}"
                        href="{{ route('pasca_home_auditor', 'standar') }}">Ketercapaian standar</a>
                </div>
            </div>
                ...
                <div class="col-auto text-left box mr-3">
                    ...
                    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="moduleDropDown">
                        @foreach ($years as $year)
                            <a class="dropdown-item {{ Request::is(['pasca_auditor/evaluasi/' . $year, 'pasca_auditor/standar/' . $year, 'pasca_auditor/semua/' . $year]) ? 'active' : '' }}"
                                href="@if ($keterangan == 'Evaluasi diri') {{ route('pasca_year_auditor', ['evaluasi', $year]) }} @elseif ($keterangan == 'Ketercapaian standar') {{ route('pasca_year_auditor', ['standar', $year]) }} @else {{ route('pasca_year_auditor', ['semua', $year]) }} @endif">{{ $year }}</a>
                        @endforeach
                    </div>
                </div>
            @endif
