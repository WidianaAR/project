...
@section('isi')
    ...
    <div class="row m-0 align-items-center">
        <div class="col pl-1">
            <span class="text-muted">Daftar pasca / Evaluasi / <a href="">{{ $data->prodi->nama_prodi }}
                    {{ $data->tahun }}</a></span>
        </div>
        @can('auditor')
            @if ($data->status_id != 7)
        ...
        <a href="@if (Auth::user()->role_id == 4) {{ route('pasca_home_auditor') }} @else {{ route('pasca_home') }} @endif"
            class="btn btn-sm btn-secondary"><i class="fa fa-arrow-left" aria-hidden="true"></i>Kembali</a>
    </div>
